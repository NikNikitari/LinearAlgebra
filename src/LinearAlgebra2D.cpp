...
Vector2D::Vector2D(const float vector[2]) : x(vector[0]), y(vector[1]) {}
...
Vector2D Vector2D::operator+(const Vector2D &vector) {return Vector2D(x+vector[0], y+vector[1]);}
float Vector2D::operator*(const Vector2D &vector) {return x * vector[0] + y * vector[1];}

Matrix2D::Matrix2D(): matrix{0}{}
Matrix2D::Matrix2D(const float _matrix[2][2]) {
	matrix[0] = _matrix[0][0];
	matrix[1] = _matrix[0][1];
	matrix[2] = _matrix[1][0];
	matrix[3] = _matrix[1][1];
}
Matrix2D::Matrix2D(const float& a, const float& b, const float& c, const float& d) {
	matrix[0] = a;
	matrix[1] = b;
	matrix[2] = c;
	matrix[3] = d;
...
const float &Matrix2D::operator[](int index) const {
	if (index > 3 || index < 0) throw std::out_of_range("Index out of range");
	return matrix[index];
}

const float &Matrix2D::operator()(int x, int y) const {
